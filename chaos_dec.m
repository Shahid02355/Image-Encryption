...
shuffled=reshape(xorr1R,rowR,colR);
 for j=1:1:colR
    for i=1:1:rowR
        if(mod(l(j),2)==0)
           if((i+l(j))<=rowR)           %shift down of column
               sh_col(i+l(j),j)=shuffled(i,j);
               col_shift_even(i,j)=i+l(j);
               else
               sh_col((i+l(j)-rowR),j)=shuffled(i,j); 
           ...
           if((i-l(j))>=1)          %shift up of column
                ...
                sh_col((rowR+i-l(j)),j)=shuffled(i,j);
    ...
    for j=1:1:colR
        if(mod(k(i),2)==0)
            if((j-k(i))>=1)       %shift left of row
                ...
                sh_row(i,(colR+j-k(i)))=sh_col(i,j);
            ...
            if((j+k(i))<=colR)   %shift right of row
               ...
               sh_row(i,(j+k(i)-colR))=sh_col(i,j); 
               row_shift_even(i,j)=(j+k(i)-colR);
            end
        end
    end
end
image_decryption=sh_row;
